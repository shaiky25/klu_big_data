...
Close the pulse with one push notification summarizing: sessions built (with their flagged assumptions), sessions flagged for re-review, and sessions that failed validation (with the reason). The first time you ever build a deck, mention the standing Keynote-import known gap (a lint-clean deck doesn't guarantee it opens in Keynote) once; check `build-state.json` for whether you've already mentioned it before repeating it — the owner asked not to hear it every time.
